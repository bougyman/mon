...
#
# $Id: Client.pm,v 1.4 2011/06/20 17:26:39 trockij Exp $
#
...
BEGIN {
    @ISA = qw(Exporter);
    @EXPORT_OK = qw(%OPSTAT %OPSTAT_ATOI %OPSTAT_ITOA $VERSION);
}
...
%OPSTAT = ("fail" => $STAT_FAIL, "ok" => $STAT_OK, "coldstart" =>
   ...
   "configerr" => $STAT_CONFIGERR);

%OPSTAT_ATOI = %OPSTAT;

%OPSTAT_ITOA = (
    $STAT_FAIL => "fail", $STAT_OK => "ok",
    $STAT_COLDSTART => "coldstart", $STAT_WARMSTART => "warmstart",
    $STAT_LINKDOWN => "linkdown", $STAT_UNKNOWN => "unknown",
    $STAT_TIMEOUT => "timeout", $STAT_UNTESTED => "untested",
    $STAT_DEPEND => "dependency", $STAT_CONFIGERR => "configerr"
);
...
sub new {
    ...
    if ( (defined ($ENV{"USER"}) ) && ($ENV{"USER"} ne "") ){
    	...
    	if ($^O ne "MSWin32") { #Win32 doesn't have getpwuid :(
	    $self->{"USERNAME"} = eval( (getpwuid ($<))[0] );
	}
    }
...
sub get {
    ...
    return _un_esc_str (split /\s+/, $var, 1);
}
...
I<linkdown>, I<unknown>, I<timeout>,  I<untested>, I<configerr>.
