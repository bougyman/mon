...
#
# ----------------------------------------------------------------------
# $Id: mon.cgi,v 1.6 2011/06/20 17:26:25 trockij Exp $
# ----------------------------------------------------------------------
#
#
# INSTRUCTIONS
...
$RCSID = '$Id: mon.cgi,v 1.6 2011/06/20 17:26:25 trockij Exp $';
$RCSVERSION = '$Revision: 1.6 $';
$VERSION = $RCSVERSION;
...
#
$moncgi_config_file = "/var/www/html/mon.cgi.cf";
...
sub list_status {
	...
	foreach $service (sort keys %{$op{$group}}) {     #begin service loop
	    ...
	    } elsif ($s->{"opstatus"} == $OPSTAT{"configerr"})  {
		$td_bg_color = ($service_disabled_string eq "") ?  $unchecked_color : $disabled_color ;
		$webpage->print("<td align=left bgcolor=\"$td_bg_color\">\n");
		$webpage->print("$service_disabled_string<a href=\"$url?${monhost_and_port_args}command=svc_details&amp;args=$group,$service\">");
		$webpage->print("<b>${service}</b>");
		$webpage->print("<br><font>(CONFIGERR)</font></a>${desc_string}\n");
		$webpage->print("</td>\n");
